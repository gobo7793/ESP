	...
	void Update () 
	{
		...
		{						 //the garbage collection memory problem arises from local alloction of memory, and not freeing it up efficiently
			...
			case "c":
				...
				break;*/
			case "f":
				animator.SetTrigger("Attack");//the animator controller will detect this non combat bool, and go into a non combat state "in" this weaponstate
				break;
            case "b":
                animator.SetTrigger("Buecken");//the animator controller will detect this non combat bool, and go into a non combat state "in" this weaponstate
                break;


            case "w":
                movementTargetPosition = transform.position + new Vector3(0, 0, -0.1f);
                wasAttacking = false;
                animator.SetFloat("Walking", -1.0f);

                break;
            case "a":
                movementTargetPosition = transform.position + new Vector3(0.1f, 0, 0);
                wasAttacking = false;
                break;
            case "s":
                movementTargetPosition = transform.position + new Vector3(0, 0, 0.1f);
        ...
        //float v = Vector3.Distance(movementTargetPosition, transform.position);
